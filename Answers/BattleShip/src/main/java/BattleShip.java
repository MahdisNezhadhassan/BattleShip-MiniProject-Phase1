...
public class BattleShip {
    ...
    static void placeShips(char[][] grid) {
        ...
        for (int i = 0; i < shipSize.length; i++) {
            ...
            while (!placed) {
                ...
                if (canPlaceShip(grid, row, col, size, vertical)) {
    ...
    static void playerTurn(char[][] opponentGrid, char[][] trackingGrid) {
        int row, col;
        while (true) {
            System.out.println("Enter the location of your attack (e.g., A5):");
            String input = scanner.nextLine();
            if (isValidInput(input)) {
                row = input.charAt(0) - 'A';
                col = Integer.parseInt(input.substring(1)) - 1;  // Fixed column input to be between 0 and 9
                if (trackingGrid[row][col] == '~') {
                    ...
                    System.out.println("You attacked this location before! Try again.");
                }
            } else {
    ...
    static boolean isValidInput(String input) {
        ...
        int col = colChar - '0';
        return col >= 1 && col <= 10;  // Corrected column validation (1-10 instead of 0-9)
    }
